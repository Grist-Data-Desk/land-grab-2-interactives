...
const generateBoundsByTribe = (
  links: FeatureCollection<LineString>
): Record<string, turf.BBox> => {
  const linksByTribe = _.groupBy(
    links.features,
    (feature) => feature.properties.present_day_tribe
  );

  const boundsByTribe = Object.entries(linksByTribe).reduce(
    (acc, [tribe, links]) => {
      const bounds = turf.bbox(turf.featureCollection(links));
    ...
    {} as Record<string, turf.BBox>
  );

  // Add bounds for 'All'.
  boundsByTribe.All = turf.bbox(links);

  return boundsByTribe;
};
...
const main = async (): Promise<void> => {
  ...
  console.log('Generating bound for parcels by tribe.');
  const boundsByTribe = generateBoundsByTribe(links);

  console.log('Writing bounds to data/processed/bounds-by-tribe.json.');
  await fs.writeFile(
    path.resolve(__dirname, '../data/processed/bounds-by-tribe.json'),
    JSON.stringify(boundsByTribe, null, 2)
  );
};
