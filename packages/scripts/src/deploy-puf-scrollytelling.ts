...
const SCROLLYTELLING_PATH = 'land-grab-ii/dev/puf-scrollytelling/dist/assets';

/**
 * Delete the previous build artifacts of the puf-scrollytelling package.
 *
 * @param s3Client – The S3 client to use for deleting objects.
 */
const deleteBuild = async (s3Client: S3Client): Promise<void> => {
  try {
    // List all objects in the puf-scrollytelling/dist/assets directory.
...
/**
 * Deploy the source code located at `packages/puf-scrollytelling/dist/assets` to the
...
const main = async (): Promise<void> => {
  ...
  const files = await fs.readdir(
    path.resolve(__dirname, '../../puf-scrollytelling/dist/assets'),
    { recursive: true }
  );

  console.log(
    `Uploading build artifacts from packages/puf-scrollytelling/dist/assets.`
  );
  for (const file of files) {
    const Body = await fs.readFile(
      path.resolve(__dirname, '../../puf-scrollytelling/dist/assets/', file)
    );
